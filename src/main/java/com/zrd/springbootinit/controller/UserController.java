...
public class UserController {
    ...
    public BaseResponse<Boolean> userGetCheckNumber(@RequestParam String phoneNumber)
    {
        if (StrUtil.isBlank(phoneNumber)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"手机号不能为空");
        }
        if (!isValidPhoneNumber(phoneNumber)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"请输入有效的手机号");
        }
        Boolean result = userService.userGetCheckNumber(phoneNumber);
        return ResultUtils.success(result,"验证码已发送");
    }
    ...
    public BaseResponse<Boolean> userLogin(@RequestBody UserLoginRequest userLoginRequest, HttpServletRequest request) {
        ...
        String checkNumber = userLoginRequest.getCheckNumber();
        String phoneNumber = userLoginRequest.getPhoneNumber();
        if (StringUtils.isAnyBlank(checkNumber, phoneNumber)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        if (!isValidPhoneNumber(phoneNumber)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"请输入有效的手机号");
        }
        Boolean result = userService.userLogin(phoneNumber, checkNumber, request);
